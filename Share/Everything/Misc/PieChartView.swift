...
class PieChartView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        func createCircleMaskUsingCenterPoint(point: CGPoint, radius: Float) -> UIImage {
            ...
            ctx2.addArc(center: CGPoint(x:point.x,y:point.y),
                        radius: CGFloat(radius),
                        startAngle: 0.0,
                        endAngle: (360.0)*CGFloat(M_PI)/180.0,
                        clockwise: false)
            ctx2.closePath()
            ctx2.fillPath()
        ...
        func createGradientImageUsingRect(rect: CGRect) -> UIImage {
            ...
            let topCenterPoint = CGPoint(x:currentBounds.midX, y:currentBounds.maxY);
            let bottomCenterPoint = CGPoint(x:currentBounds.midX,y: currentBounds.minY);
            ctx3!.drawLinearGradient(gradient!, start: topCenterPoint, end: bottomCenterPoint, options: CGGradientDrawingOptions(rawValue: UInt32(0)));
